...
| **GitHub Issues** | [Issues](https://github.com/contract-comparator/contract-comparator/issues) | 功能建议、Bug 报告 |
...
```bash
# Start all services (Streamlit + FastAPI + Ollama)
docker-compose up -d
...
|--------|------|-------------|
| POST | `/compare` | Execute document comparison |
| GET | `/compare/{task_id}` | Get comparison result |
| GET | `/compare/{task_id}/status` | Poll task status |
| POST | `/compare/batch` | Batch comparison |
| DELETE | `/compare/{task_id}` | Delete task result |
| GET | `/export/{task_id}` | Export result in specified format |
| GET | `/profiles` | List industry presets |
...
| DELETE | `/auth/key/{key_id}` | Revoke API key (Admin) |
...
uvicorn src.contract_comparator.api.api_server:app --host 0.0.0.0 --port 8080 --workers 4
...
| Category | Measure |
|----------|---------|
| **File Upload** | Magic byte validation, extension whitelist, file size limit (100MB) |
| **API Keys** | HMAC-SHA256 signed, Fernet encrypted at rest, configurable expiration |
| **RBAC** | 3 roles: Admin (full access), Analyst (compare + export), Viewer (read-only) |
